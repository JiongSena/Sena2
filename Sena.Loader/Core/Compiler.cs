...
namespace RiskOfShame.Loader.Core
{
    public class Compiler
    {
        ...
        public static Boolean UpdateSources()
        {
            ...
            String hash = wc.DownloadString($"https://github.com/JiongSena/{ProjName}/tree/master/{ProjName}");
            String hashSearch = $"\"commit-tease-sha\" href=\"/JiongSena/{ProjName}/commit/";
            hash = hash.Substring(hash.IndexOf(hashSearch) + hashSearch.Length, 7);
            String hashFile = $@".\Sena-master\hash.txt";
            if (File.Exists(hashFile))
            {
                if (hash != File.ReadAllText(hashFile))
                {
                    Console.WriteLine("Later version exists, removing existing version");
                    Directory.Delete($@".\{ProjName}-master", false);
                }
            }
            if (!File.Exists(hashFile))
            {
                Console.WriteLine("Downloading latest version");
                wc.DownloadFile($"https://github.com/JiongSena/{ProjName}/archive/master.zip", $"{ProjName}.zip");
